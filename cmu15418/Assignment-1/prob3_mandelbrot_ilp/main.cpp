...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < numRuns; ++i) {
        ...
        numIters = mandelbrotSerial(mandel_ref, x0, y0, x1, y1, width, iheight,
                                    0, iheight, maxIterations, output_ref);
    ...
    for (int j = 0; par_funs[j].unrollCount > 0; j++) {
        ...
        for (int i = 0; i < numRuns; ++i) {
            double startSeconds = CycleTimer::currentSeconds();
            numIters = mandelbrotParallel(
                par_funs[j].fun, par_funs[j].unrollCount, x0, y0, x1, y1, width,
                iheight, 0, height, maxIterations, output_par);
            double endSeconds = CycleTimer::currentSeconds();
