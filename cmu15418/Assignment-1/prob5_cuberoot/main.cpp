...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        cuberootSerial(N, initialGuess, values, output);
        double endTime = CycleTimer::currentSeconds();
        minSerial = std::min(minSerial, endTime - startTime);
    }

    printf("[cuberoot serial]:\t\t[%.3f] ms\n", minSerial * 1000);
    ...
    for (int i = 0; i < 3; ++i) {
        double startTime = CycleTimer::currentSeconds();
        cuberoot_ispc(N, initialGuess, values, output);
        double endTime = CycleTimer::currentSeconds();
        minISPC = std::min(minISPC, endTime - startTime);
    }

    printf("[cuberoot ispc]:\t\t[%.3f] ms\n", minISPC * 1000);
    ...
    for (int i = 0; i < 3; ++i) {
        double startTime = CycleTimer::currentSeconds();
        cuberoot_ispc_withtasks(N, initialGuess, values, output);
        double endTime = CycleTimer::currentSeconds();
        minTaskISPC = std::min(minTaskISPC, endTime - startTime);
    }

    printf("[cuberoot task ispc]:\t[%.3f] ms\n", minTaskISPC * 1000);
